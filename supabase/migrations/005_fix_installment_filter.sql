-- Fix: max_price_installment ahora representa la cuota MENSUAL máxima del usuario.
-- Se compara contra price_installment / installment_count (cuota mensual del producto),
-- no contra price_installment total. Si installment_count es NULL se omite el filtro.
...
AS $$
  ...
  WHERE
    ...
    AND (
      ...
      OR p.price_installment IS NULL
      OR p.installment_count IS NULL
      OR (p.price_installment / p.installment_count) <= max_price_installment
    )
